...
library(MASS)
library(lme4)
data1=read.csv("rawdata1.csv")
row.names(data1)=data1$X
data1=as.data.frame(data1[,-1])
```

##Select standard deviations, you can change the mean and the standard deviations as you want

```{r}
sd=as.data.frame(apply(data1,1,sd))
mean=as.data.frame(apply(data1,1,mean))
colnames(sd)="sd"
colnames(mean)="mean"
data1$sd=sd$sd
data1$mean=mean$mean
sub=as.data.frame(data1[data1$mean>=0.4&data1$mean<=0.6&data1$sd>=0.8&data1$sd<=1.2,])#select the value you want 
sub <- sub[order(sub$mean),]
sub=sub[,1:(length(sub)-2)]
```

Construct counts, genes, ids, library sizes.
```{r}
...
png(file="corr2.png", res=300, width=4500, height=4500)
...
png(file="hist_low_log.png", res=300, width=2000, height=2000)
...
for (i in 1:p){
  for (j in 1:p){
    if (i < j){
      ...
      data3=sub1[((j-1)*n+1):(j*n),]$count
      data3_Lib_Size=sub1[((i-1)*n+1):(i*n),]$Lib_Size
      result4[i,j]=cor(data2/data2_Lib_Size,data3/data3_Lib_Size)
    }
  }
} 
```
...
png(file="corr4.png", res=300, width=4500, height=4500)
